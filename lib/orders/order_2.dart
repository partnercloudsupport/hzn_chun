...
class OrderSecondPage extends StatefulWidget{
  ...
  OrderSecondPage(this.state);

  @override
  State<StatefulWidget> createState() {
    return new OrderSecondWidget(state);
  }

}


class OrderSecondWidget extends State<OrderSecondPage>{

  var state;
  var pageNum = 0;

  OrderSecondWidget(this.state);
...
//        }));
        ...
        Container(
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              Container(
                child: Text("已付款",style: TextStyle(fontSize: 14,color: Colors.deepOrangeAccent),)
              ),
  ...
  Future<Null> loadOrderData(int state, int pageNum) async {
    ...
    var resData = await HttpUtil.getInstance().get(I_ORDER_LIST,context,data: formData);
